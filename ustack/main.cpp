...
void show_dev(struct rte_eth_dev_info * dev_info)
{
...
int main(int argc, char **argv) {
    ...
    struct rte_mempool *q1_mbuf_pool = rte_pktmbuf_pool_create("mbuf_pool1", nb_mbufs,
                                                MEMPOOL_CACHE_SIZE, 0, RTE_MBUF_DEFAULT_BUF_SIZE,
                                                rte_socket_id());
    // struct rte_mempool *q2_mbuf_pool = rte_pktmbuf_pool_create("mbuf_pool2", nb_mbufs,
    //                                             MEMPOOL_CACHE_SIZE, 0, RTE_MBUF_DEFAULT_BUF_SIZE,
    //                                             rte_socket_id());
    if (/*q2_mbuf_pool == NULL || */q1_mbuf_pool == NULL)
    ...
    ret = rte_eth_dev_configure(port_id, 1, 0, &local_port_conf);
    if (ret < 0)
    ...
    ret = rte_eth_rx_queue_setup(port_id, 0, MAX_RX_QUEUE_PER_LCORE,
                                 rte_eth_dev_socket_id(port_id),
                                 &rxq_conf, q1_mbuf_pool);
    if (ret < 0)
